 ...
 *{box-sizing:border-box}
 ...
 .railhead{padding:10px 12px;border-bottom:1px solid #e3e8ef}
 .railhead b{font-size:14px}.railhead input{width:100%;margin-top:6px;font-size:14px;padding:7px;border:1px solid #cdd5e0;border-radius:7px}
 #tree{flex:1;overflow:auto;padding:6px 10px}
 ...
 .schhead{font-weight:700;color:#14213d;margin:8px 0 3px;cursor:pointer;font-size:13px}
 .schhead .cnt{color:#9aa3b2;font-weight:400}
 ...
 .railctl{margin-top:6px;display:flex;gap:6px;align-items:center;flex-wrap:wrap;font-size:13px}
 .railctl button{padding:3px 8px}
 ...
 .tab{background:#fff;color:#14213d;border:1px solid #cdd5e0;padding:4px 12px;border-radius:7px;font-size:13px}
 ...
 #entlistpanel{flex:1;overflow:auto;padding:4px 8px}
 ...
 .erow .ev{color:#5a6478;font-variant-numeric:tabular-nums}.erow .pp{color:#1b7f3b;font-weight:700;cursor:pointer}
 ...
 .rail.floating .railhead{background:#f2f5f9;border-radius:10px 10px 0 0}
 ...
 .frow .lab{flex:1;min-width:280px}.frow .val{font-variant-numeric:tabular-nums}
 .vcell{width:92px;flex:none;text-align:right;font-variant-numeric:tabular-nums;color:#14213d}
 ...
 body.dark .railtabs{background:#121a26;border-color:#2a3547}
 body.dark .railhead,body.dark .modalhead{border-color:#2a3547}
 body.dark .rail{border-right-color:#2a3547}
 body.dark select,body.dark input,body.dark textarea{background:#1b2433;color:#e6e9ef;border-color:#2a3547}
 body.dark button.sec{background:#1b2433;color:#e6e9ef;border-color:#2a3547}
 body.dark .tab{background:#1b2433;color:#e6e9ef;border-color:#2a3547}body.dark .tab.on{background:#1b7f3b;color:#fff;border-color:#1b7f3b}
 ...
 body.dark .schhead,body.dark .card .v,body.dark .frow .val,body.dark .vcell,body.dark td:first-child{color:#e6e9ef}
 ...
 body.dark .chip{background:#1b2433;border-color:#2a3547}
 .erow.on{background:#dcefe2}
body.dark .erow.on{background:#16361f}
 body.dark .trow:hover,body.dark .erow:hover{background:#1f2a3a}body.dark .trow.on{background:#16361f}
 body.dark .erow{border-color:#222c3b}body.dark .frow{border-color:#222c3b}
...
#charttip{position:fixed;pointer-events:none;z-index:50;background:var(--tip-bg,#1a2535);color:var(--tip-fg,#e6e9ef);border:1px solid #3a4a5e;border-radius:8px;padding:8px 12px;font-size:13px;line-height:1.55;white-space:normal;max-width:min(460px,82vw);display:none;box-shadow:0 4px 16px rgba(0,0,0,.4)}
...
#lbl-overlay::after{content:"\2921";position:absolute;right:4px;bottom:1px;font-size:11px;line-height:1;color:var(--muted,#9aa3b2);opacity:.5;pointer-events:none}
 ...
 .chartbox{position:relative;resize:both;overflow:hidden;width:100%;max-width:100%;min-width:300px;min-height:200px;box-sizing:border-box;border-radius:8px;margin-bottom:2px}
 ...
 <div class="rail">
  ...
  <div id="panelItems">
   <div class="railhead"><span class="muted">click to add to chart</span>
    ...
    <div class="railctl">
     <button id="drilldn" class="sec" title="expand one level in open branches [ ] keys">▼ Drill</button>
  ...
  <div id="panelEnts" style="display:none">
   <div class="railhead"><button id="entdetach" class="sec" title="use Items + Entities at once">⧉ Detach</button><span class="muted">click to chart · ＋peer to bucket</span>
    <input id="entsearch" placeholder="search bank / RSSD…">
    <div class="railctl">Show
     <select id="entfilter"><option value="all">All</option><option value="bank">Individual banks</option><option value="agg">Aggregates</option><option value="peer">Peer groups</option><option value="charted">Charted only</option><option value="watchlist">★ Watchlist</option></select>
     Sort
     <select id="entsort"><option value="assets">Total assets</option><option value="deposits">Total deposits</option><option value="loans">Total loans</option><option value="equity">Total equity</option><option value="current">Current measure</option><option value="name">Name A–Z</option><option value="rssd">RSSD</option></select>
     <label><input type="checkbox" id="entdesc" checked> high→low</label>
     <!-- showmerged: lineage not available for this form --><label title="show predecessor / merged-bank RSSDs in entity list"><input type="checkbox" id="showmerged"> Show merged</label>
    </div></div>
   <div id="entlistpanel"><p class="muted" style="padding:10px">…</p></div>
  </div>
 </div>
 ...
 <div class="main">
  ...
  <div class="row">
   ...
   <div><button id="add" class="sec">+ Add to chart</button> <button id="addpeer" class="sec">+ Add to peer</button>
    ...
    <button id="reportbtn" class="sec" disabled title="Select a single bank to generate a tear-sheet report">📋 Report</button> <button id="exportbtn" class="sec">⬇ Export</button> <button id="copylink" class="sec" title="Copy link to current chart state">🔗 Link</button> <button id="kbdbtn" class="sec" title="Keyboard shortcuts (?)">⌨</button></div>
  </div>
  ...
  <div><label>Measures (click items in the left rail; ✕ to remove)</label> <button id="clrmeas" class="sec" style="padding:1px 6px;font-size:12px;opacity:0.6" title="Remove all measures">✕ Clear</button> <button id="calcbtn" class="sec" style="padding:1px 6px;font-size:12px" title="Define a custom calculated series combining existing line items">Σ Calc</button>
   ...
   <div id="calcdiv" style="display:none;margin:4px 0 8px;padding:8px 10px;border:1px solid var(--border,#ccc);border-radius:6px;font-size:13px;background:var(--bg2,#f7f9fb)">
    ...
    <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap;align-items:center">
     <button id="calcSave" class="sec" style="font-size:13px;padding:2px 8px" title="Download all custom formulas as ffiec_call_formulas.json">⬇ Save to file</button>
     <button id="calcLoad" class="sec" style="font-size:13px;padding:2px 8px" title="Load formulas from a saved ffiec_call_formulas.json file">⬆ Load from file</button>
  ...
  <details class="box" id="peerbox"><summary><b>Peer-group builder</b> — custom bucket of banks; aggregates correctly for levels &amp; ratios</summary>
...
<div id="formmodal" class="modal" style="display:none"><div class="modalbox">
 <div class="modalhead" style="flex-wrap:wrap;row-gap:4px"><b>Call-report view</b>
  ...
  <label style="font-size:13px">From <select id="ffrom"></select></label>
  <label style="font-size:13px">To <select id="fto"></select></label>
  ...
  <button id="fexp" class="sec">⊕</button><button id="fcol" class="sec">⊖</button>
  <button id="fpop" class="sec" title="float / dock">⧉</button>
...
try{window._inlineLbls=localStorage.getItem('ffiec_call_inlinelbls')==='1';const _cs=localStorage.getItem('ffiec_call_chartsize');if(_cs){const _m=_cs.split('x');window._chartW=+_m[0]||0;window._chartH=+_m[1]||0;}window._lblBoxLeft=+localStorage.getItem('ffiec_call_lblboxleft')||null;window._lblBoxTop=+localStorage.getItem('ffiec_call_lblboxtop')||null;window._lblBoxW=+localStorage.getItem('ffiec_call_lblboxw')||0;window._lblBoxH=+localStorage.getItem('ffiec_call_lblboxh')||0;}catch(_){}
...
const DERIV={
 'S_CASH':{type:'sum',lbl:'Liquidity ▸ Cash & balances due $ (0081+0071)',plus:['0081','0071']},
 'S_LIQ':{type:'sum',lbl:'Liquidity ▸ Liquid assets $ (cash+securities+ff sold)',plus:LIQ},
 'D_LIQ_ASSETS':{type:'ratio',lbl:'Liquidity ▸ Liquid assets / Assets (%)',plus:LIQ,den:['2170']},
 ...
 'D_NPL_CI':{type:'ratio',lbl:'Loan quality ▸ C&I NPL % (Past Due + Non Accrual / C&I loans)',plus:['1250','1252','1253','1254','1255','1256'],den:['1763','1764']},
 'D_NPL_CC':{type:'ratio',lbl:'Loan quality ▸ Credit card NPL % (Past Due + Non Accrual / CC loans)',plus:['B575','B576','B577'],den:['B538']},
 'D_NPL_AUTO':{type:'ratio',lbl:'Loan quality ▸ Auto NPL % (Past Due + Non Accrual / auto loans)',plus:['K213','K214','K215'],den:['K137']},
 'D_NPL_CONS':{type:'ratio',lbl:'Loan quality ▸ Other consumer NPL % (Past Due + Non Accrual / other consumer)',plus:['B578','B579','B580'],den:['K207']},
 'D_NPL_AG':{type:'ratio',lbl:'Loan quality ▸ Agricultural NPL % (Past Due + Non Accrual / ag loans)',plus:['1594','1597','1583'],den:['1590']},
 'D_NPL_RES':{type:'ratio',lbl:'Loan quality ▸ 1-4 family RE NPL % (Past Due + Non Accrual / residential loans)',plus:['5398','5399','5400','C236','C237','C229','C238','C239','C230'],den:['1797','5367','5368']},
 'D_NPL_CONSTR':{type:'ratio',lbl:'Loan quality ▸ Construction & land NPL % (Past Due + Non Accrual / construction loans)',plus:['F172','F173','F174','F176','F175','F177'],den:['F158','F159']},
 'D_NPL_MULTI':{type:'ratio',lbl:'Loan quality ▸ Multifamily RE NPL % (Past Due + Non Accrual / multifamily loans)',plus:['3499','3500','3501'],den:['1460']},
 'D_NPL_CRE':{type:'ratio',lbl:'Loan quality ▸ CRE nonfarm nonres NPL % (Past Due + Non Accrual / CRE loans)',plus:['F178','F179','F180','F182','F181','F183'],den:['F160','F161']},
 'S_NCO':{type:'sum',lbl:'Subtotal ▸ Net charge-offs $ YTD (charge-offs-recoveries)',plus:['4635'],minus:['4605']},
 'S_UNUSED':{type:'sum',lbl:'Subtotal ▸ Total unused commitments $ (RC-L item 1)',plus:UNUSED},
 'S_EXPOSURE':{type:'sum',lbl:'Subtotal ▸ Total loan exposure $ (loans+unused)',plus:['2122',...UNUSED]},
 'D_NIM':{type:'ratio',lbl:'Earnings ▸ Net interest margin % approx. (NII/assets, annualized)',plus:['4074'],den:['2170'],annualize:true},
 'D_EFF':{type:'ratio',lbl:'Earnings ▸ Efficiency ratio % (nonint expense / (NII+nonint income))',plus:['4093'],den:['4074','4079']},
 // Hidden hybrid_sum entries (no lbl → excluded from ★ shelf by buildTree filter)
 // RCON/RCFD items are coalesced as COMB<base> in the tree; key on COMBHK05 not RCONHK05.
 'COMBHK05':{type:'hybrid_sum',parts:[{reported:'RCONHK05',components:['RCONHK06','RCONK220']}]},
 'RCFN2200':{type:'hybrid_sum',parts:[{reported:'RCFN2200',components:['RCFN6631','RCFN6636']}]},
 // RC-N item 9 totals — gap 2001-Q1→2016-Q4 filled by summing RC-N items 1-8 leaf codes (per column).
 // Validation: 0.25%/0.15%/0.80% mean error vs reported in post-gap overlap 2017-Q1→2026-Q1.
 // Caveat: COMBK213-K218 (consumer auto/other) absent pre-2011-Q1 → ~4% understatement 2001-2010.
 'COMB1406':{type:'hybrid_sum',parts:[{reported:'COMB1406',components:['COMBF172','COMBF173','COMB3493','COMB5398','COMBC236','COMBC238','COMB3499','COMBF178','COMBF179','RCFNB572','COMB5377','COMB5380','COMB1594','COMB1251','COMB1254','COMB1606','COMBB575','COMBK213','COMBK216','COMB5389','COMB5459','COMBF166','COMBF169']}]},
 'COMB1407':{type:'hybrid_sum',parts:[{reported:'COMB1407',components:['COMBF174','COMBF175','COMB3494','COMB5399','COMBC237','COMBC239','COMB3500','COMBF180','COMBF181','RCFNB573','COMB5378','COMB5381','COMB1597','COMB1252','COMB1255','COMB1607','COMBB576','COMBK214','COMBK217','COMB5390','COMB5460','COMBF167','COMBF170']}]},
 'COMB1403':{type:'hybrid_sum',parts:[{reported:'COMB1403',components:['COMBF176','COMBF177','COMB3495','COMB5400','COMBC229','COMBC230','COMB3501','COMBF182','COMBF183','RCFNB574','COMB5379','COMB5382','COMB1583','COMB1253','COMB1256','COMB1608','COMBB577','COMBK215','COMBK218','COMB5391','COMB5461','COMBF168','COMBF171']}]},
};
...
const VIEWS=[
 ...
 {id:'credit',name:'Credit quality',icon:'⚠️',desc:'NPL %, noncurrent ratio, reserve coverage, net charge-off rate.',measures:['D_NPL','D_NONCUR_PCT','D_RESNPL','D_NCO_LOANS'],xform:{},range:'5Y'},
 ...
 {id:'funding',name:'Funding mix',icon:'💰',desc:'Brokered deposits, borrowings, non-deposit liabilities, deposits/assets.',measures:['D_BROKERED','D_BORROW_ASSETS','D_NONDEP','D_DEPASSETS'],xform:{},range:'5Y'},
 {id:'loanbook',name:'Loan quality by book',icon:'📊',desc:'NPL % broken out by loan book: C&I, credit card, auto, consumer, residential, CRE.',measures:['D_NPL_CI','D_NPL_CC','D_NPL_AUTO','D_NPL_CONS','D_NPL_RES','D_NPL_CRE'],xform:{},range:'5Y'},
 {id:'balsheet',name:'Balance sheet levels (peer-scale)',icon:'📈',desc:'Liquid assets, cash, borrowings, noncurrent loans — sized by ÷ assets for cross-bank comparison.',measures:['S_LIQ','S_CASH','S_BORROW','S_NONCUR'],xform:{normbyassets:true,normden:'COMB2170'},range:'5Y'},
 {id:'growth',name:'Growth / trend',icon:'📉',desc:'Assets, loans, deposits, equity indexed to 100 — long-run growth trend.',measures:['COMB2170','COMB2122','COMB2200','COMB3210'],xform:{idx:true},range:'10Y'},
 {id:'reserve',name:'Reserve build',icon:'🛡️',desc:'Reserves/loans, reserve coverage, provisions/loans, net charge-offs.',measures:['D_RESLOANS','D_RESNPL','D_PROV_LOANS','D_NCO_LOANS'],xform:{},range:'10Y'},
 {id:'undrawn',name:'Undrawn / utilization',icon:'🧮',desc:'Unused commitments vs loans and assets, loan utilization rate.',measures:['D_UND_LOANS','D_UND_ASSETS','D_UTIL'],xform:{},range:'5Y'},
 {id:'peercompare',name:'Peer compare — size bucket',icon:'⚖️',desc:'NPL % as a starting lens for comparing against ALL, 031/041/051, or a peer group you pick.',measures:['D_NPL'],xform:{},range:'5Y'},
];
const DYN={};   // dynamic subtotal measures created by clicking a grouping row in the tree
const USERCALC={};  // user-defined custom calculated series (persisted via Save formulas → localStorage)
...
const SUB_AGG_DESCS={
  ...
  // RC-Q, RC-R, RC-R I/I-B, RC-R II: non-additive (fair-value Total+Levels = 2x; risk-weight
...
// ── Custom calculated series (Σ Calc) — ported from 002 ──
...
// HIGH-2: isAggScope covers ALL, charter-type groups, size buckets, and multi-member peer groups.
const isAggScope=id=>id==='ALL'||id==='031'||id==='041'||id==='051'||id.startsWith('SIZE_')||(id.startsWith('PEER:')&&(peers[id.slice(5)]||[]).length>1);
const isRawPct=m=>PCTC.has(m)&&!DERIV[m];
...
async function seriesFor(id,m){const ids=expand(id);if(!ids.length)return [];
 ...
 if(d&&d.type==='hybrid_sum'){
   const allCodes=[...new Set(d.parts.flatMap(p=>[p.reported,...p.components]))];
   const r=(await conn.query(`SELECT entity_id,quarter_end,mdrm,value FROM t WHERE entity_id IN (${sqlList(ids)}) AND mdrm IN (${sqlList(allCodes)})`)).toArray();
   ...
   const out=[];for(const q of Object.keys(byqe).sort()){let num=0,anyN=false;
     for(const eid of Object.keys(byqe[q])){const mp=byqe[q][eid];
       let pnum=0,pany=false;
       for(const part of d.parts){const rv=mp[part.reported]??null;
         if(rv!=null){pnum+=rv;pany=true;}else{const[cs,csSeen]=acc(mp,part.components);if(csSeen){pnum+=cs;pany=true;}}}
       if(pany){num+=pnum;anyN=true;}}
     if(anyN)out.push([q,num]);}
   _seriesCache.set(_sk,out);return out;}
 if(d){const bases=[...d.plus,...(d.minus||[]),...(d.den||[])];
   const codes=[];for(const b of bases)for(const p of['COMB','RCFD','RCON','RIAD','RCFA','RCFW','RCOA','RCOW','RCFN'])codes.push(`${p}${b}`);
...
async function init(){try{
 ...
 try{const hr=await fetch(new URL('ffiec_call_hierarchy.json',location.href).href);if(hr.ok)HIER=await hr.json();}catch(e){}
 ...
 document.getElementById('expall').onclick=()=>expandAll(true);
 document.getElementById('colall').onclick=()=>expandAll(false);
 document.getElementById('drilldn').onclick=()=>drillSmart(1);
 document.getElementById('drillup').onclick=()=>drillSmart(-1);
 ...
 (function(){const app=document.querySelector('.app'),rail=document.querySelector('.rail'),head=rail.querySelector('.railtabs');
  ...
  head.addEventListener('mousedown',e=>{if(!rail.classList.contains('floating'))return;if(e.target.closest('input,button,label'))return;
    drag=true;dx=e.clientX-rail.offsetLeft;dy=e.clientY-rail.offsetTop;e.preventDefault();});
  window.addEventListener('mousemove',e=>{if(!drag)return;rail.style.left=(e.clientX-dx)+'px';rail.style.top=(e.clientY-dy)+'px';});
 ...
 document.getElementById('calcadd').onclick=()=>{
   ...
   try{localStorage.setItem('ffiec_call_formulas',getFormulasJson());}catch(_){}
   _seriesCache.clear();
...
function renderChips(){const c=document.getElementById('chips');if(!active.length){c.innerHTML='<span class="muted">none</span>';const cl=document.getElementById('crosslinks');if(cl)cl.innerHTML='';return;}
 ...
 const cl=document.getElementById('crosslinks');if(cl){const banks=active.filter(a=>a.id.startsWith('BANK:'));if(banks.length){const base=location.href.replace(/\/[^/]*$/,'/');const eHash=banks.map(a=>a.id).join(',');const mk=(dir,lbl)=>`<a href="${base.replace(/site_call\//,''+dir+'/')}index.html#e=${encodeURIComponent(eHash)}" target="_blank" style="color:var(--muted,#9aa3b2);text-decoration:underline dotted">${lbl}</a>`;cl.innerHTML='Also view in: '+mk('site_fry9c','Y-9C')+' · '+mk('site_002','002');}else cl.innerHTML='';}
...
function loadFavs(){try{return new Set(JSON.parse(localStorage.getItem('ffiec_call_favs')||'[]'));}catch{return new Set();}}
function saveFavs(s){localStorage.setItem('ffiec_call_favs',JSON.stringify([...s]));}
function loadWL(){try{return new Set(JSON.parse(localStorage.getItem('ffiec_call_wl')||'[]'));}catch{return new Set();}}
function saveWL(s){localStorage.setItem('ffiec_call_wl',JSON.stringify([...s]));}
...
// ---- tree (persistent rail, collapsible hierarchy, one consolidated row per item) ----
const REPORT=/^(RCON|RCFD|RCFN|RCFA|RCFW|RCOA|RCOW|RIAD)[A-Z0-9]{4}$/;
...
function rowEl(nd,has,dispCap){
 ...
 if(nd.header){
   ...
   return d;}
 const d=document.createElement('div');d.className='trow'+(nd.comb?' comb':'')+(nd.header?' hdr':'');
 d.dataset.code=nd.code;d.dataset.txt=(nd.code+' '+nd.caption).toLowerCase();d.dataset.depth=nd.depth;
 d.style.paddingLeft=(6+(nd.depth-1)*14)+'px';
 const car=`<span class="caret"${has?'':' style="visibility:hidden"'}>▸</span>`;
 const cap=`<span class="cap" title="${String(nd.caption||'').replace(/"/g,'&quot;')}">${dispCap||nd.caption||''}</span>`;
...
// like RC-N where ordering is column-major and a depth-stack would mis-parent rows.
...
function emitSchedule(sch,showRaw){const allRows=HIER[sch];
 // Emit rows in ORIGINAL hierarchy order, interleaving placeholder/header rows and code rows, so
 // code-bearing top-level items don't sort above header-only items (the RAL hoisting class). nest()
 // orders siblings by list position, so preserving JSON order here keeps numeric form order.
 const raw=allRows.filter(r=>REPORT.test(r.mdrm));
 const capByBase={};for(const r of raw){const b=r.mdrm.slice(4);if(r.caption&&!capByBase[b])capByBase[b]=r.caption;}
 ...
 for(const r of allRows){
   if(!REPORT.test(r.mdrm)){
     ...
     else out.push({code:'EMPTY:'+r.item,caption:'(empty)',num:r.item||'',depth:r.depth||1,comb:false,derived:false,pct:false,placeholder:true,sch:sch});
     continue;}
   const p=r.mdrm.slice(0,4),base=r.mdrm.slice(4),cap=r.caption||capByBase[base]||'';if(!cap)continue;
   if(r.item==null)continue;
   const depth=r.depth||1;
   if(p==='RCFD'||p==='RCON'){
     if(!done.has('C'+base)){done.add('C'+base);out.push({code:'COMB'+base,caption:cap,num:r.item||'',depth,comb:true,derived:false,pct:false,col:!!r.col,sch:sch});}
     if(showRaw&&!done.has(r.mdrm)){done.add(r.mdrm);out.push({code:r.mdrm,caption:cap+' ['+p.slice(2)+']',num:'',depth:depth+1,comb:false,derived:false,pct:false,sch:sch});}
   } else if(p==='RCFN'){ if(combBases.has(base))continue;
     if(!done.has(r.mdrm)){done.add(r.mdrm);out.push({code:r.mdrm,caption:cap,num:r.item||'',depth,comb:false,derived:false,pct:false,col:!!r.col,sch:sch});}
   } else { if(!done.has(r.mdrm)){done.add(r.mdrm);out.push({code:r.mdrm,caption:cap,num:r.item||'',depth,comb:false,derived:false,pct:false,col:!!r.col,sch:sch});} }}
 return out;}
...
function filterTree(q){q=q.trim().toLowerCase();
 if(!q){document.querySelectorAll('#tree .trow').forEach(r=>r.style.display='');
   document.querySelectorAll('#tree .schsec').forEach(s=>s.style.display='');expandAll(false);return;}
...
const _pctB=new Set([...PCTC].map(c=>c.slice(4)));
// Roll-up de-dup rules: clicking a header with an explicit rule uses the pre-aggregated total
// code instead of summing children (prevents double-counting gross+net+HFS or parts+total).
const FV_SCHED=new Set(['RCQ']);
const ROLLUP_RULES={
 'RC':{'4':{codes:['RCFD5369','RCFDB529']}},
 'RI':{'1':{codes:['RIAD4107']},'2':{codes:['RIAD4073']},'5':{codes:['RIAD4079']},'7':{codes:['RIAD4093']}}
};
function descCodes(nd){
 const sch=nd.sch||'';
 const rules=ROLLUP_RULES[sch]||{};
 const fv=FV_SCHED.has(sch);
 // RCB: Col A (HTM amortized) + Col D (AFS fair value); RCT: Col A only ($ total, not counts); else: pair (drop domestic RCON when RCFD consolidated exists)
 const colStrat=sch==='RCB'?'AD':sch==='RCT'?'A':'pair';
 const ok=c=>c&&!c.placeholder&&!c.derived&&!_pctB.has(c.code.slice(4));
 const pickCols=cl=>{
   const cs=cl.filter(ok);if(!cs.length)return [];
   if(colStrat==='A')return [cs[0].code.slice(4)];
   if(colStrat==='AD'){const codes=cs.map(c=>c.code);return codes.length>=4?[codes[0].slice(4),codes[codes.length-1].slice(4)]:[codes[0].slice(4)];}
   const consol=new Set(cs.filter(c=>c.code.startsWith('RCFD')||c.code.startsWith('COMB')).map(c=>c.code.slice(4)));
   return cs.filter(c=>!(c.code.startsWith('RCON')&&consol.has(c.code.slice(4)))).map(c=>c.code.slice(4));};
 const out=[];
 const findItem=(n,it)=>{if(n.num===it)return n;for(const c of(n.children||[])){const r=findItem(c,it);if(r)return r;}return null;};
 (function rec(n){
   const r=rules[n.num];
   if(r){
     if(r.suppress)return;
     if(r.codes){for(const c of r.codes)out.push(c.slice(4));return;}
     if(r.use){const t=findItem(nd,r.use);if(t&&t!==n)rec(t);return;}
   }
   if(fv){
     const cols=(n.children||[]).filter(c=>c.col&&!c.header&&c.code&&!c.placeholder&&!c.derived);
     if(cols.length){out.push(cols[0].code.slice(4));}
     for(const c of(n.children||[])){if(c.header)rec(c);else if(!c.col&&c.code&&!c.placeholder&&!c.derived&&!_pctB.has(c.code.slice(4)))out.push(c.code.slice(4));}
     return;}
   const kids=n.children||[];
   const colLeaves=kids.filter(c=>!c.header&&c.col&&c.code&&!c.placeholder&&!c.derived&&!_pctB.has(c.code.slice(4)));
   if(colLeaves.length){for(const b of pickCols(colLeaves))out.push(b);return;}
   for(const c of kids){if(c.header)rec(c);else if(c.code&&!c.placeholder&&!c.derived&&!_pctB.has(c.code.slice(4)))out.push(c.code.slice(4));}
 })(nd);
 return [...new Set(out)];}
function hasPctDesc(nd){return(function chk(n){for(const c of n.children||[]){if(c.header){if(chk(c))return true;}else if(c.code&&_pctB.has(c.code.slice(4)))return true;}return false;})(nd);}
function markTree(){const on=new Set(measures.map(m=>m.code));
 document.querySelectorAll('#tree .trow').forEach(r=>r.classList.toggle('on',on.has(r.dataset.code)||on.has('SUB:'+r.dataset.code)));}
...
function entCat(k){return k==='bank'?'bank':(k==='all'||k==='filing_type'||k==='size_bucket')?'agg':'other';}
async function computeSortVals(field){
 if(field===entSortField)return;
 entSortVals=new Map();
 if(field==='name'||field==='rssd'){entSortField=field;return;}
 const baseMap={assets:'2170',deposits:'2200',loans:'2122',equity:'3210'};
 let code; if(field==='current'){if(!measures.length){entSortField=field;return;}code=measures[0].code;} else code='COMB'+baseMap[field];
 let base=null; if(code.startsWith('COMB')||/^(RCFD|RCON)/.test(code))base=code.slice(4);
 // LOW-1: derived code (D_* ratio/subtotal) — use perFilerValues for sort instead of raw query
 if(!base&&field==='current'){
   try{const latR=(await conn.query('SELECT max(quarter_end) q FROM t')).toArray();const latQ=latR.length?String(latR[0].q):null;
     if(latQ){const vals=await perFilerValues(code,[latQ]);const cur=vals[latQ]||new Map();
       for(const[id,v]of cur)entSortVals.set(id,v);}}catch(e){}
   entSortField=field;return;}
 const where=base?`mdrm IN ('COMB${base}','RCFD${base}','RCON${base}')`:`mdrm='${code}'`;
 try{const r=(await conn.query(`SELECT entity_id, mdrm, value FROM t WHERE ${where} AND quarter_end=(SELECT max(quarter_end) FROM t)`)).toArray();
   const byid={};for(const x of r){(byid[x.entity_id]=byid[x.entity_id]||{})[x.mdrm]=Number(x.value);}
   for(const id in byid){const mp=byid[id];const v=base?coalesce(mp,base):mp[code];if(v!=null)entSortVals.set(id,v);}}catch(e){}
 entSortField=field;}
async function renderEntList(){const field=document.getElementById('entsort').value;await computeSortVals(field);
 ...
 // showmerged: lineage not available for FFIEC 031 — toggle has no visible effect
 const wlSet=loadWL();
 const pool=[];for(const e of (window._ents||[]))pool.push({id:e.entity_id,label:e.lbl,cat:entCat(e.k)});
 ...
 const rows=[];for(const p of pool){if(filt==='charted'){if(!active.some(a=>a.id===p.id))continue;}else if(filt==='watchlist'){if(!wlSet.has(p.id))continue;}else if(filt!=='all'&&p.cat!==filt)continue;
   ...
   let sv;if(field==='name')sv=p.label.toLowerCase();
   else if(field==='rssd'){const m=p.id.match(/(\d+)/);sv=m?+m[1]:(desc?-Infinity:Infinity);}
   else sv=entSortVals.has(p.id)?entSortVals.get(p.id):(desc?-Infinity:Infinity);
   rows.push({...p,sv});}
 ...
 cont.innerHTML=rows.slice(0,600).map(r=>{const val=(field==='name')?'':(field==='rssd'?(r.id.match(/(\d+)/)?r.id.match(/(\d+)/)[1]:''):(entSortVals.has(r.id)?fmtUnit(entSortVals.get(r.id),false):''));
   const isOn=active.some(a=>a.id===r.id);const isWL=wlSet.has(r.id);
   return `<div class="erow${r.cat==='agg'?' agg':''}${isOn?' on':''}" data-id="${r.id}" data-label="${r.label.replace(/"/g,'&quot;')}"><span class="en">${r.label}</span><span class="ev">${val}</span> <span class="pp" title="add to peer builder">＋peer</span><span class="rpt" title="Open entity report">📋</span><span class="ewl${isWL?' on':''}" title="Toggle watchlist">★</span></div>`;}).join('')||'<p class="muted" style="padding:8px">none match</p>';
 cont.querySelectorAll('.erow').forEach(el=>{const id=el.dataset.id,label=el.dataset.label;
   ...
   el.querySelector('.pp').onclick=ev=>{ev.stopPropagation();if(!peerMembers.find(a=>a.id===id))peerMembers.push({id,label});renderPeerBuilder();document.getElementById('peerbox').open=true;};
...
async function recompute(){
 if(!measures.length||!active.length){lastSeries=[];Qall=[];draw();stateToHash();return;}
 const mySeq=++_rcSeq;
 ...
 lastSeries=out;
 const qs=new Set();for(const s of out)for(const r of s.rows)qs.add(r[0]);
 _assetRows.clear();const _ndEl=document.getElementById('normden');const _ndCd=_ndEl?_ndEl.value:'COMB2170';window._normDenCd=_ndCd;for(const e of active){if(mySeq!==_rcSeq)return;const ar=await seriesFor(e.id,_ndCd);_assetRows.set(e.id,Object.fromEntries(ar.map(r=>[r[0],r[1]])));}
 Qall=[...qs].sort(); rangeSel={a:0,b:Math.max(0,Qall.length-1)}; syncSlider(); await recomputeExtraCharts(); draw();stateToHash();}
function syncSlider(){const n=Qall.length;const w=document.getElementById('sliderwrap');
 if(n<2){w.style.display='none';return;}w.style.display='flex';
...
function renderLblOverlay(series,show){
  ...
  if(!box){
    ...
    window.addEventListener('mouseup',()=>{if(!_lblDrag)return;_lblDrag=null;
      try{localStorage.setItem('ffiec_call_lblboxleft',box.offsetLeft);localStorage.setItem('ffiec_call_lblboxtop',box.offsetTop);}catch(_){};});
    new ResizeObserver(()=>{try{localStorage.setItem('ffiec_call_lblboxw',Math.round(box.offsetWidth));localStorage.setItem('ffiec_call_lblboxh',Math.round(box.offsetHeight));}catch(_){}}).observe(box);
...
function draw(){const host=document.getElementById('panes');
 ...
 const workSeries=normOn?lastSeries.map(s=>{if(s.pct)return s;const am=_assetRows.get(s.eid)||{};const norm=s.rows.map(([q,v])=>{const a=am[q];return [q,v!=null&&a&&a!==0?100*v/a:null];});return {...s,rows:norm,pct:true,label:s.label+' / '+_ndLbl+' %'};}).map(s=>({...s})):lastSeries;
 const groups=[['$ thousands',workSeries.filter(s=>!s.pct)],['percent',workSeries.filter(s=>s.pct)]];
 let html='';
 ...
 if(hasDol&&hasPct){
   const axisUI=dualAxis?workSeries.map((s,i)=>`<label style="font-size:12px;white-space:nowrap;margin-right:6px"><input type="checkbox" class="ax-right" data-i="${i}" ${window._axisRight.has(s.label)?'checked':''}> <span style="color:${s.color}">${(s.label||'').slice(0,30)}</span> → right</label>`).join(''):'';
 ...
 document.getElementById('cards').innerHTML=
  ...
  `<div class=card><div class=k>QoQ${pQ&&prev!=null?' vs '+pQ:''}</div><div class="v ${cls(qoq)}">${arr(qoq)}</div>${qoq!=null?`<div class="muted" style="font-size:13px;margin-top:2px">${absChg(qoqRaw)}</div>`:''}</div>`+
  `<div class=card><div class=k>YoY${yQ&&yr!=null?' vs '+yQ:''}</div><div class="v ${cls(yoy)}">${arr(yoy)}</div>${yoy!=null?`<div class="muted" style="font-size:13px;margin-top:2px">${absChg(yoyRaw)}</div>`:''}</div>`+
  `<div class=card><div class=k>Total Δ (range${f0q?' from '+f0q:''})</div><div class="v ${cls(tot)}">${arr(tot)}</div>${tot!=null?`<div class="muted" style="font-size:13px;margin-top:2px">${absChg(totRaw)}</div>`:''}</div>`+
  `<div class=card><div class=k>Series</div><div class=v>${lastSeries.length}</div></div>`;
 // table (wide: quarter + each series)
...
function pane(series,pct,unit,win,stacked){const W=1080,H=300,pad=64,n=win.length;
 ...
 if(stacked){
   ...
   for(let si=0;si<series.length;si++){const s=series[si];const sm=sMaps[si];
     ...
     if(lastTopCx!=null){const pts2=s.label.split(' \xb7 ');const nE=active.length,nM=measures.length;const sl=(nE>1&&nM===1?pts2[0]:(nE===1&&nM>1?short(pts2.slice(1).join(' \xb7 '))||short(s.label):short(s.label))).slice(0,34);_el.push({x:lastTopCx+5,y:lastTopCy+4,sl,color:s.color});}}
 }else{
 for(const s of series){let p='',firstCx,lastCx,lastCy;s.rows.forEach((r,k)=>{const cx=X(xi[r[0]]).toFixed(1),cy=Y(r[1]).toFixed(1);if(k===0)firstCx=cx;lastCx=cx;lastCy=cy;p+=(k?'L':'M')+cx+' '+cy+' ';
   ...
   if(lastCx!=null){const pts2=s.label.split(' \xb7 ');const nE=active.length,nM=measures.length;const sl=(nE>1&&nM===1?pts2[0]:(nE===1&&nM>1?short(pts2.slice(1).join(' \xb7 ')):short(s.label))).slice(0,34);_el.push({x:+lastCx+5,y:+lastCy+4,sl:sl,color:s.color});}}
 } // end if(stacked)/else
...
function _applyLinkedTfm(tfm){['idx','qoqdelta','yoydelta','roll4q','sharemode','normbyassets','stackedmode'].forEach(id=>{const el=document.getElementById(id);if(el)el.checked=!!(id==='normbyassets'?tfm.normByAssets:id==='stackedmode'?tfm.stacked:tfm[id]);});if(tfm.normDen){const nde=document.getElementById('normden');if(nde&&nde.value!==tfm.normDen){nde.value=tfm.normDen;window._normDenCd=tfm.normDen;try{localStorage.setItem('ffiec_call_normden',tfm.normDen);}catch(_){}}}  _extraCharts.forEach(c=>{c.transforms={...c.transforms,...tfm};});}
...
// sum-then-divide ratio logic matching seriesFor(). RIAD income codes (4340,4635,4605,4230)
...
async function perFilerValues(measCode, quarters){
 // map: quarter -> Map(entity_id -> per-bank value) for a COMB/raw code OR a DERIV ratio/sum/hybrid
 const out={}; for(const q of quarters) out[q]=new Map();
 let d=DERIV[measCode]||DYN[measCode]; if(!d&&measCode.startsWith('COMB'))d={type:'sum',plus:[measCode.slice(4)],minus:[],den:[]};
 if(d&&(d.type==='hybrid_sum'||d.type==='hybrid_ratio')){
   ...
   const r=(await conn.query(`SELECT entity_id,quarter_end,mdrm,value FROM t WHERE kind='bank' AND mdrm IN (${sqlList(codes)}) AND quarter_end IN (${sqlList(quarters)})`)).toArray();
   const byqf={}; for(const x of r){const q=String(x.quarter_end);(byqf[q]=byqf[q]||{});(byqf[q][x.entity_id]=byqf[q][x.entity_id]||{})[x.mdrm]=Number(x.value);}
   ...
   const codes=[];for(const b of bases)for(const p of['COMB','RCFD','RCON','RIAD'])codes.push(p+b);
   const r=(await conn.query(`SELECT entity_id,quarter_end,mdrm,value FROM t WHERE kind='bank' AND mdrm IN (${sqlList(codes)}) AND quarter_end IN (${sqlList(quarters)})`)).toArray();
   const byqf={}; for(const x of r){const q=String(x.quarter_end);(byqf[q]=byqf[q]||{});(byqf[q][x.entity_id]=byqf[q][x.entity_id]||{})[x.mdrm]=Number(x.value);}
   const acc=(mp,arr)=>{let s=0,seen=false;for(const b of arr){const v=coalesce(mp,b);if(v!=null){s+=v;seen=true;}}return [s,seen];};
   for(const q of quarters){const per=byqf[q]||{};
...
async function renderLeague(){
 ...
 const spkFn=eid=>{const pts=spkQs.map(sq=>vals[sq]?vals[sq].get(eid):undefined).filter(v=>v!=null&&!isNaN(v));if(pts.length<2)return '';const mn2=Math.min(...pts),mx2=Math.max(...pts),rng=mx2-mn2||1;const W2=56,H2=20,pad2=2;const xs=pts.map((_,i)=>(pad2+i*(W2-2*pad2)/(pts.length-1)).toFixed(1));const ys=pts.map(v=>(H2-pad2-(v-mn2)/rng*(H2-2*pad2)).toFixed(1));const poly=xs.map((x,i)=>`${x},${ys[i]}`).join(' ');const lc=DK()?'#4ade80':'#1b7f3b';return `<svg width="${W2}" height="${H2}" style="vertical-align:middle"><polyline points="${poly}" fill="none" stroke="${lc}" stroke-width="1.5"></polyline></svg>`;};
 let rows=[...cur.entries()].map(([eid,v])=>{const pv=prevQ?vals[prevQ].get(eid):null, yv=yoyQ?vals[yoyQ].get(eid):null;
   const qoq=meas.pct?(pv!=null?v-pv:null):(pv?100*(v/pv-1):null);
   const yoy=meas.pct?(yv!=null?v-yv:null):(yv?100*(v/yv-1):null);
   return {eid,name:id2lbl.get(eid)||eid,v,qoq,yoy};});
 if(assetMap&&bkt){const lo=bkt==='-'?0:(+bkt)*1e9,hi=bkt==='-'?1e8:(+bkt)*1e10;rows=rows.filter(r=>{const a=assetMap.get(r.eid);return a!=null&&a>=lo&&(bkt==='-'?a<1e8:a<hi);});}
 ...
 const pctileMap=new Map(valSorted.map((r,i)=>[r.eid,nm>1?Math.round(100*(nm-1-i)/(nm-1)):50]));
...
function fentIds(){const ents=window._feEnts||[];const ids=[];for(const ent of ents)for(const id of expand(ent.id))ids.push(id);return [...new Set(ids)];}
async function openForm(){initModalResize('formmodal','ffiec_call_formmsz');if(!HIER){showToast('Hierarchy file not loaded.');return;}
 const initE=active[0]||resolveEnt();
 if(!initE&&(!window._feEnts||!window._feEnts.length)){document.getElementById('formmodal').style.display='flex';document.getElementById('formbody').innerHTML='<p class="muted">Add a bank using the input above.</p>';return;}
 ...
 if(qs.length){ft.value=qs[qs.length-1];ff.value=qs[Math.max(0,qs.length-8)];}
 document.getElementById('formmodal').style.display='flex';renderForm();}
async function renderForm(){const ids=fentIds();if(!ids.length)return;const fq=window._fq||[];
 ...
 let cols=fq.slice(lo,hi+1);const colsDesc=[...cols].reverse();
 ...
 for(const sch of keys){const items=HIER[sch].filter(rr=>REPORT.test(rr.mdrm)&&val[rr.mdrm]);if(!items.length)continue;
   ...
   const {sec,rows}=mkSec(SCHED_NAMES[sch]||sch,items.length);body.appendChild(sec);
   renderFormNodes(rows,nest(flat),colsDesc,val);
...
async function openReport(entityId){
 ...
 initModalResize('reportmodal','ffiec_call_reportmsz');
 ...
 const nm=id2lbl.get(entityId)||entityId;
 document.getElementById('rpt-title').textContent=nm+' · '+entityId;
 {const ac=document.getElementById('rpt-addchart');const on=active.some(a=>a.id===entityId);ac.textContent=on?'✓ In chart':'📈 Add to chart';ac.style.opacity=on?'0.5':'';ac.onclick=()=>{if(!active.find(a=>a.id===entityId)){active.push({id:entityId,label:nm});renderChips();scheduleRecompute();}ac.textContent='✓ In chart';ac.style.opacity='0.5';};}
 ...
 try{
  const qtrsRes=(await conn.query(`SELECT DISTINCT quarter_end FROM t WHERE entity_id='${entityId}' ORDER BY quarter_end DESC LIMIT 16`)).toArray();
...
async function buildReport(entityId,nm,latestQ,qtrs){
 const kpiCodes=['RCFD2170','RCON2170','RCFD2122','RCON2122','RCFD2200','RCON2200',
  'RCFD3210','RCON3210','RIAD4340','RIAD4074','RIAD4079','RIAD4093',
  'RIAD4635','RIAD4605','RCFD3123','RCON3123','RCFD1403','RCON1403','RCFD1406','RCON1406','RCFD1407','RCON1407',
  'RCOAP793','RCFAP793','RCON7274','RCOA7205','RCFA7205','RCON7205'];
 ...
 const dep=getV(['RCFD2200','RCON2200']);
 const eq=getV(['RCFD3210','RCON3210']);
 const ni=getV(['RIAD4340']),nii=getV(['RIAD4074']),niinc=getV(['RIAD4079']),niexp=getV(['RIAD4093']);
 const coff=getV(['RIAD4635']),rec=getV(['RIAD4605']),alll=getV(['RCFD3123','RCON3123']);
 const npl30=getV(['RCFD1406','RCON1406']),npl90=getV(['RCFD1407','RCON1407']),nona=getV(['RCFD1403','RCON1403']);
 ...
 // Capital: coalesce without RCOAH036 (confirmed absent from Call parquet)
 const cet1=getV(['RCOAP793','RCFAP793','RCON7274']);
 const tier1=getV(['RCOA7205','RCFA7205','RCON7205']);
 ...
 try{
  const rk=(await conn.query(`SELECT COUNT(*) n FROM t WHERE mdrm='RCFD2170' AND quarter_end='${latestQ}' AND kind='bank' AND value>=${assets??0}`)).toArray();
  const ct=(await conn.query(`SELECT COUNT(*) n FROM t WHERE mdrm='RCFD2170' AND quarter_end='${latestQ}' AND kind='bank'`)).toArray();
  assetRank=Number(rk[0]?.n??0);assetCount=Number(ct[0]?.n??0);
 }catch{}
 // Peer percentile bars
 const peerPctile={};
 try{
  const peerCodes=['RCFD2170','RCFD2122','RCFD2200','RCFD3210','RIAD4340','RIAD4074','RCOAP793','RCOA7205','RCFD1403','RIAD4635'];
  for(const code of peerCodes){const ev=V[code]?.[latestQ];if(ev==null)continue;
   const res=(await conn.query(`SELECT COUNT(*) n FROM t WHERE mdrm='${code}' AND quarter_end='${latestQ}' AND kind='bank'`)).toArray();
   const tot=Number(res[0]?.n||0);if(!tot)continue;
   const lwr=(await conn.query(`SELECT COUNT(*) n FROM t WHERE mdrm='${code}' AND quarter_end='${latestQ}' AND kind='bank' AND value<=${ev}`)).toArray();
 ...
 const ncoQ=q=>{const c=getV(['RIAD4635'],q),r=getV(['RIAD4605'],q),l=getV(['RCFD2122','RCON2122'],q);return c!=null&&r!=null&&l!=null&&l>0?100*(c-r)/l*(4/qnlQ(q)):null;};
 ...
 const trends=[
  {t:'Total Assets',s:[mkS(qtrs.map(q=>[q,getV(['RCFD2170','RCON2170'],q)]),COLORS[0],'Assets')],u:'$ thousands'},
  {t:'Loans & Deposits',s:[mkS(qtrs.map(q=>[q,getV(['RCFD2122','RCON2122'],q)]),COLORS[1],'Loans'),mkS(qtrs.map(q=>[q,getV(['RCFD2200','RCON2200'],q)]),COLORS[2],'Deposits')],u:'$ thousands'},
  {t:'ROA & ROE (ann. %)',s:[mkS(qtrs.map(q=>[q,annQ(['RIAD4340'],['RCFD2170','RCON2170'],q)]),COLORS[0],'ROA'),mkS(qtrs.map(q=>[q,annQ(['RIAD4340'],['RCFD3210','RCON3210'],q)]),COLORS[1],'ROE')],u:'percent'},
  {t:'NIM & Efficiency (%)',s:[mkS(qtrs.map(q=>[q,annQ(['RIAD4074'],['RCFD2170','RCON2170'],q)]),COLORS[2],'NIM'),mkS(qtrs.map(q=>[q,effQ(q)]),COLORS[3],'Efficiency')],u:'percent'},
  {t:'Capital Ratios (%)',s:[mkS(qtrs.map(q=>[q,getV(['RCOAP793','RCFAP793','RCON7274'],q)]),COLORS[4],'CET1'),mkS(qtrs.map(q=>[q,getV(['RCOA7205','RCFA7205','RCON7205'],q)]),COLORS[5],'Tier 1')],u:'percent'},
 ...
 const nar=buildNarrative({nm,latestQ,assets,assetRank,assetCount,aQoQ,aYoY,roa,roe,cet1,eff,nplRat,nplQoQ,rescov,nco,form:'FFIEC Call'});
...
function sparkline(rows,pct,color){
 ...
 const pts=filtered.map(r=>`${X(rows.indexOf(r)).toFixed(1)},${Y(r[1]).toFixed(1)}`).join(' ');
 if(!pts)return '';
...
function buildNarrative(d){
 const {nm,latestQ,assets,assetRank,assetCount,aQoQ,aYoY,roa,roe,cet1,eff,nplRat,nplQoQ,rescov,nco,form}=d;
 ...
 const p1=`${nm} is a commercial bank with ${fA(assets)||'undisclosed assets'} as of ${latestQ}${assetRank?`, ranking #${assetRank} of ${assetCount} ${form||'FFIEC Call'} filers${pct!=null?' ('+pct+'th percentile)':''}`:''}.${grTxt}`;
...
function callScopeCond(id){
 if(!id)return null;
 if(id==='ALL')return "kind='bank'";
 if(id.startsWith('BANK:'))return `entity_id='${id}' AND kind='bank'`;
 if(id.startsWith('PEER:')){const ms=(peers[id.slice(5)]||[]);if(!ms.length)return null;return `entity_id IN (${ms.map(x=>`'${x}'`).join(',')}) AND kind='bank'`;}
 return null;}
...
function ebRawCodes(){
 ...
 const out=new Set();
 for(const c of _eb.codes){out.add(c);}
 return [...out];}
function ebEntityCond(){
 if(!_eb.entities.length)return null;
 if(_eb.entities.some(e=>e.id==='ALL'))return "kind='bank'";
 const ids=new Set();
 for(const ent of _eb.entities){
   if(ent.id.startsWith('BANK:'))ids.add(ent.id);
   else if(ent.id.startsWith('PEER:')){for(const r of peers[ent.id.slice(5)]||[])ids.add(r);}
 }
 return ids.size?`entity_id IN (${[...ids].map(x=>`'${x}'`).join(',')}) AND kind='bank'`:null;}
function pivotWide(rows){
 ...
 const byE={};for(const r of rows){const k=r.entity_id;if(!byE[k])byE[k]={nm:r.entity_label,m:{}};(byE[k].m[r.mdrm]=byE[k].m[r.mdrm]||{})[String(r.quarter_end)]=r.value;}
 ...
 return {headers:['entity_id','entity_label','mdrm','caption',...qtrs],body};}
async function ebEstimate(){
 let nC=0;
 if(_eb.scope==='schedules')nC=ebScheduleCodes().length;
 else if(_eb.scope==='codes')nC=ebRawCodes().length;
 else try{const r=(await conn.query("SELECT COUNT(DISTINCT mdrm) n FROM t WHERE kind='bank'")).toArray();nC=Number(r[0]?.n||0);}catch{nC=0;}
 let nQ=0;
 if(_eb.fromQ&&_eb.toQ)try{const r=(await conn.query(`SELECT COUNT(DISTINCT quarter_end) n FROM t WHERE kind='bank' AND quarter_end>='${_eb.fromQ}' AND quarter_end<='${_eb.toQ}'`)).toArray();nQ=Number(r[0]?.n||0);}catch{}
 let nE=_eb.entities.length||0;
 if(_eb.entities.some(e=>e.id==='ALL'))try{const r=(await conn.query("SELECT COUNT(DISTINCT entity_id) n FROM t WHERE kind='bank'")).toArray();nE=Number(r[0]?.n||0);}catch{}
 else if(nE>0){const ids=new Set();for(const ent of _eb.entities){if(ent.id.startsWith('BANK:'))ids.add(ent.id);else if(ent.id.startsWith('PEER:')){for(const r of peers[ent.id.slice(5)]||[])ids.add(r);}}if(ids.size)nE=ids.size;}
...
async function renderExportUI(){
 ...
 body.innerHTML=`<div style="border:1px solid var(--border,#ccc);border-radius:6px;padding:10px 14px;margin-bottom:8px"><b>Entities</b> <span class="muted" style="font-size:13px">Add one or more banks, ALL, or peer groups</span>
  ...
  <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
   ...
   <button id="eb-ent-all" class="sec" title="Export all filing banks">All banks</button>
 ...
 function addEbEnt(v){v=v.trim();if(!v)return;const cv=v.replace(/^★\s*/,'');
  ...
  if(lbl2id.has(v)){const id=lbl2id.get(v);if(!_eb.entities.find(e=>e.id===id))_eb.entities.push({id,label:v});renderEbChips();ebEstimate();return;}
  const m=v.match(/(\d{3,})/);if(m){const id='BANK:'+m[1];if(!_eb.entities.find(e=>e.id===id))_eb.entities.push({id,label:id2lbl.get(id)||id});renderEbChips();ebEstimate();return;}
 ...
 (function(){const inp=document.getElementById('eb-ent'),res=document.getElementById('eb-ent-res');if(!inp||!res)return;
   function buildEntCands(q){const lq=(q||'').toLowerCase();const cands=[];
     if(!lq||'all'.startsWith(lq))cands.push({v:'ALL',l:'All banks'});
     for(const n in peers){if(cands.length>=20)break;if(!lq||n.toLowerCase().includes(lq))cands.push({v:'★ '+n,l:'★ '+n+' (peer)'});}
     for(const e of (window._ents||[])){if(e.k!=='bank')continue;if(cands.length>=20)break;const lbl=e.lbl||'';if(!lq||lbl.toLowerCase().includes(lq)||e.entity_id.includes(lq))cands.push({v:lbl,l:lbl});}
     return cands;}
 ...
 document.getElementById('eb-ent-all').onclick=()=>{_eb.entities=[{id:'ALL',label:'All banks'}];renderEbChips();ebEstimate();};
 ...
 try{
  ...
  const cond=ec0||"kind='bank'";
  const res=(await conn.query(`SELECT DISTINCT quarter_end FROM t WHERE ${cond} ORDER BY quarter_end`)).toArray();
...
async function runExport(preview=false){
 ...
 // Partition: DERIV/DYN/COMB codes -> seriesFor() for faithful computed values; pure MDRM -> raw SQL.
 let derivKeys=[],rawOnly=null,mdrmF='';
 if(_eb.scope==='codes'){
   derivKeys=[..._eb.codes].filter(c=>DERIV[c]||DYN[c]||c.startsWith('COMB'));
   rawOnly=[..._eb.codes].filter(c=>!DERIV[c]&&!DYN[c]&&!c.startsWith('COMB'));
   ...
   derivKeys=[];rawOnly=[];
   for(const c of cs){const cb=(c.startsWith('RCFD')||c.startsWith('RCON'))?'COMB'+c.slice(4):c;
     if(DERIV[cb]||DYN[cb])derivKeys.push(cb);
     else if(DERIV[c]||DYN[c])derivKeys.push(c);
     else rawOnly.push(c);}
   if(rawOnly.length)mdrmF=`AND mdrm IN (${rawOnly.map(m=>`'${m}'`).join(',')}) `;}
 const rows=[];
 ...
 if(derivKeys.length){
   ...
   for(const ent of _eb.entities){
     const eid=ent.id.startsWith('BANK:')?ent.id:null;
     const nm=ent.label||'';
     for(const code of derivKeys){
       ...
       for(const[q,v]of series){
         ...
         if(preview&&rows.length>=50)break;
         rows.push({quarter_end:q,entity_id:eid,entity_label:nm,mdrm:code,value:v});}}}}
 if(_eb.fmt==='wide'&&!preview)return pivotWide(rows);
 return {headers:['quarter_end','entity_id','entity_label','mdrm','caption','value'],body:rows.map(r=>[r.quarter_end,r.entity_id,r.entity_label,r.mdrm,fullCap(r.mdrm)||'',r.value]),sql:''};}
...
(function(){
  ...
  function showTip(e,svg){
    ...
    if(window._hybridOnChart){const _rn=[];if(measures.some(m=>m.code==='COMBHK05')&&q>='2011-03-31'&&q<='2016-12-31')_rn.push('RC-E M.1.c from K220\xb15.5%');if(measures.some(m=>m.code==='RCFN2200')&&q>='1983-06-30'&&q<='1983-12-31')_rn.push('RC 13.b from 6636');const _rcn9=(m=>m==='COMB1406'||m==='COMB1407'||m==='COMB1403');if(measures.some(m=>_rcn9(m.code))&&q>='2001-03-31'&&q<='2016-12-31'){const _early=q<='2010-12-31';_rn.push('RC-N item 9 from items 1-8 components \xb1'+(measures.some(m=>m.code==='COMB1406')?'0.25':'0.15-0.80')+'%'+(_early?' (lower bound ~4%, consumer sub-codes absent 2001-2010)':''));}if(_rn.length)html+=`<div style="font-size:11px;color:#64748b;margin-top:2px;font-style:italic">⚠ Reconstructed: ${_rn.join('; ')}</div>`;}
  ...
  document.getElementById('panes').addEventListener('click',e=>{
    ...
    if(window._pinnedQ){
      ...
      tip.innerHTML+=`<div style="font-size:10px;color:#9aa3b2;margin-top:3px">📌 click same date (or Esc) to unpin <span id="charttip-close" title="Unpin" style="cursor:pointer;float:right;opacity:0.7">✕</span></div>`;
